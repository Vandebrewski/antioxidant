...
.service('PostService', function ($rootScope, $http, $q, WORDPRESS_API_URL){
	...
	this.getRecentPosts = function(page) {
		...
		$http.jsonp("http://www.new-impulse.com/tag/antioxidant/?json=get_posts&callback=JSON_CALLBACK")
...
.service('PostService2', function ($rootScope, $http, $q, WORDPRESS_API_URL2){

	this.getRecentPosts = function(page) {
		var deferred = $q.defer();

		$http.jsonp("http://www.new-impulse.com/tag/superfood/?json=get_posts&callback=JSON_CALLBACK")
		.success(function(data) {
			deferred.resolve(data);
		})
		.error(function(data) {
	...
	this.getPost = function(postId) {
		...
		$http.jsonp(WORDPRESS_API_URL2 + 'get_post/' +
		'?post_id='+ postId +
		'&callback=JSON_CALLBACK')
		.success(function(data) {
			deferred.resolve(data);
